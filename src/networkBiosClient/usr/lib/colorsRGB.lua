...
-- thanks, added some functions, ben (infinity)
...
colorsRGB.RGB = function (name)
	if not colorsRGB.list[name] then
		return { r = 0, g = 0, b = 0 }
	end
	return { r = colorsRGB.list[name][1], g = colorsRGB.list[name][2], b = colorsRGB.list[name][3]}
end
...
colorsRGB.HEX = function (name)
	...
	return (colorsRGB.list[name][1] + (256 * colorsRGB.list[name][2]) + (256 * 256 * colorsRGB.list[name][3]))
end

colorsRGB.RGB2HEX = function (r, g, b)
