...
require ("package").loaded.marketGUIex = nil
...
function touchEvent(id, device, x, y, button, user)
  ...
  if newTimer > 0 then 
	--_G.gui[i].ui.gpu.set(1, 25, " set timer "..newTimer.." for ".._G.gui[i].ui.currentScreen.." ")
	_G.gui[i].touchTimer = event.timer(newTimer, function() _G.gui[i]:printTrades() end) 
  end  
  
  if false and timerCanceled == true then _G.gui[i].ui.gpu.set(1, 24, " canceled a timer ") end  
end
...
function init()
  for i=1,#_G.config.terminals do
