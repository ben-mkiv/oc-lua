...
while n.getFacing() ~= sides.north do d.turn(false) end	
if skip < 1 then
while pos[3] > 0 do if not d.move(2) then break end
		pos[3]=pos[3]-1 end	
	while pos[3] < 0 do if not d.move(3) then break end
		pos[3]=pos[3]+1 end	
	end
...
if tonumber(d.count(inventory_slot)) > 0 then
return false
end
d.swing(side)
if tonumber(d.count(inventory_slot)) == 1 then 
return true 
end
end 
return false		
...
gotoWoot("controller")
while n.getFacing() ~= wootOrientation do d.turn(true) end
